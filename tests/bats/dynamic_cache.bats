...
# Publishing writes a temp then renames it. The writer traps EXIT to reap
# its own temp, so a successful write leaves the script and nothing else.
@test "publishing leaves no temp files behind" {
  run_session <<'EOS'
source <(antidote init)
antidote bundle foo/bar &>/dev/null
antidote bundle foo/baz &>/dev/null
temps=($ANTIDOTE_HOME/.dynamic/*.new.*(N))
scripts=($ANTIDOTE_HOME/.dynamic/[0-9]*.zsh(N))
echo "temps: $#temps"
echo "scripts: $#scripts"
EOS
  assert_line "temps: 0"
  assert_line "scripts: 2"
}
