    ...
    if (( $#o_pin )); then
      ...
      if [[ "$ANTIDOTE_LOCKFILE_DISABLED" != true ]]; then
        ...
        if [[ -n "$lock_commit" ]] && [[ "$(git_sha "$bundle_path")" != "$lock_commit" ]]; then
          ...
          git_checkout_detach "$bundle_path" "$lock_commit" 2>/dev/null \
            || warn "antidote: warning: could not checkout locked commit $lock_commit for $bname"
        fi
      fi
    fi
  fi
...
lockfile_write() {
  ...
  lines=(
    "#!/usr/bin/env zsh"
    "# antidote lockfile — generated by antidote bundle"
    "# Do not edit manually. Run 'antidote update' to refresh."
  )
