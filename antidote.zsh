...
### Canonical bundle identifier: site/owner/repo (path-style independent).
bundle_id() {
  local url=$(tourl "$1")
  url=${url%.git}
  if [[ $url == https://* ]]; then
...
zsh_script() {
  ...
  local repat bundle bname bundle_path btype dopts zsh_defer zsh_defer_bundle giturl current_pin lock_commit
  ...
  if [[ "$btype" == (repo|url|ssh_url) ]] && [[ ! -e "$bundle_path" ]]; then
    ...
    if (( $#o_pin )); then
      ...
      # Check lockfile: if a lock-commit exists, checkout that SHA
      if [[ "$ANTIDOTE_LOCKFILE_DISABLED" != true ]]; then
        zstyle -s ":antidote:bundle:$(bundle_id $bundle)" lock-commit lock_commit
        if [[ -n "$lock_commit" ]] && [[ "$(git_sha "$bundle_path")" != "$lock_commit" ]]; then
          git_fetch "$bundle_path" --depth 1 origin "$lock_commit" 2>/dev/null \
            || git_fetch "$bundle_path" --unshallow origin 2>/dev/null
          if ! git_checkout_detach "$bundle_path" "$lock_commit" 2>/dev/null; then
            warn "antidote: error: locked commit $lock_commit not found for $bname — refusing to load"
            del "$bundle_path"
            return 1
          fi
        fi
  ...
  if [[ "$btype" == (repo|url|ssh_url) ]] && [[ -e "$bundle_path" ]]; then
    if (( $#o_pin )); then
      ...
      # Sync existing repo to lockfile SHA if needed
      if [[ "$ANTIDOTE_LOCKFILE_DISABLED" != true ]]; then
        zstyle -s ":antidote:bundle:$(bundle_id $bundle)" lock-commit lock_commit
        if [[ -n "$lock_commit" ]] && [[ "$(git_sha "$bundle_path")" != "$lock_commit" ]]; then
          git_fetch "$bundle_path" --depth 1 origin "$lock_commit" 2>/dev/null \
...
### Write the lockfile with current bundle SHAs.
lockfile_write() {
  local bundledir url id sha new_content
  local -a lines bundles

  [[ "$ANTIDOTE_LOCKFILE_DISABLED" == true ]] && return 0

  lines=(
    "#!/usr/bin/env zsh"
    "# antidote lockfile — generated by 'antidote bundle' or 'antidote update'"
  ...
  for bundledir in ${(o)bundles}; do
    ...
    lines+=("zstyle ':antidote:bundle:${id}' lock-commit '${sha}'")
  done

  new_content="${(pj:\n:)lines}"
  if [[ -r "$ANTIDOTE_LOCKFILE" ]] && [[ "$(< "$ANTIDOTE_LOCKFILE")" == "$new_content" ]]; then
    return 0
  fi
  printf '%s\n' $lines >| "$ANTIDOTE_LOCKFILE"
}
...
() {
  ...
  zstyle -s ':antidote:lockfile'     file         ANTIDOTE_LOCKFILE     || ANTIDOTE_LOCKFILE=${ZDOTDIR:-$HOME}/.zsh_plugins.lock
  ANTIDOTE_LOCKFILE=${~ANTIDOTE_LOCKFILE}
  ...
  # Source lockfile to load lock-commit zstyles
  if [[ "$ANTIDOTE_LOCKFILE_DISABLED" != true ]] && [[ -r "$ANTIDOTE_LOCKFILE" ]]; then
    source "$ANTIDOTE_LOCKFILE"
  fi
} "${0:A}"
