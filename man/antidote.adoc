...
== Synopsis
*antidote* [-v | --version] [-h | --help] [--diagnostics] <command> [<args> ...]
...
=== Dynamic Bundling
Users familiar with legacy plugin managers like Antigen might prefer to use dynamic bundling. With dynamic bundling you avoid having separate plugin files. To use dynamic bundling, we need to change how *antidote bundle* handles your plugins. We do this by sourcing the output from *antidote init*.

Each *antidote bundle* line caches its generated load script in a file under *antidote home*, so after the first run new shells load the cache instead of regenerating. Set `zstyle ':antidote:dynamic' zcompile 'yes'` to zcompile those cache files. The cache refreshes automatically when antidote or your config changes, when ':antidote:*' zstyles change, or after *antidote purge* or *antidote update*.
...
The bundle directory naming style in ANTIDOTE_HOME can be controlled with the following `zstyle`:
...
Bundles are cloned shallow so the first run is fast, then deepened by a background fetch that nothing waits on. To hold a bundle at its shallow clone and skip that fetch:

----
zstyle ':antidote:bundle:*' shallow yes                       # every bundle
zstyle ':antidote:bundle:romkatv/powerlevel10k' shallow yes   # just one
----

The default is `no`. A shallow bundle only gives up the list of individual commits in the *antidote-update* report. See *antidote-bundle*(1).
...
Bundles can be held back from freshly pushed commits with the following `zstyle`, which takes a number of days:

----
zstyle ':antidote:bundle:*' min-age 7
zstyle ':antidote:bundle:zsh-users/zsh-autosuggestions' min-age 30
----

It defaults to unset, and holding bundles back costs a full-history clone. Since commit dates are set by whoever writes the commit, `min-age` is a cushion rather than a guarantee. See *antidote-bundle* for the details, and use `pin:` when you need a commit fixed for certain.
...
It defaults to unset, and holding bundles back costs a full-history clone. A plugin with no commit that old is still installed at its latest commit, so `min-age` never blocks a new plugin. Since commit dates are set by whoever writes the commit, `min-age` is a cushion rather than a guarantee. See *antidote-bundle* for the details, and use `pin:` when you need a commit fixed for certain.
...
== Controlling Output
Output in antidote can be customized as well.

Colorization of certain output is supported in antidote, and color support detection
is controlled by the *COLORTERM* variable, which modern terminals should set. If you
want to override that behavior, setting the following variables can help control that:

*NO_COLOR*:: Set 'NO_COLOR=1' to disable color in output.
*CLICOLOR_FORCE*:: Set 'CLICOLOR_FORCE=1' to force color in output. NO_COLOR must be 0 or unset.

Additionally, JSON Lines (JSONL) output is supported for certain commands such as
'antidote list'. If you need JSONL support, look for this flag:
*-j, --jsonl*:: Output in JSONL.
