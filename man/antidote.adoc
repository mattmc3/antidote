...
Each *antidote bundle* line caches its generated load script in a zcompiled file under *antidote home*, indexed by a manifest, so after the first run new shells load the cache instead of regenerating. The cache refreshes automatically when antidote or your config changes, when ':antidote:*' zstyles change, or after *antidote purge* or *antidote update*.
