...
Your `.zsh_plugins.txt` file supports directives and annotations. The default directive is `bundle`, so a bare entry like `ohmyzsh/ohmyzsh` is equivalent to `bundle:ohmyzsh/ohmyzsh`. Annotations tell antidote how to do things like load plugins from alternate paths. This lets you use plugins from popular frameworks like Oh-My-Zsh:
...
You can also use a `using:` directive to set context once and then list shorthand words:
...
See *antidote-bundle* for full directive and annotation behavior, including `using:` inheritance details.
