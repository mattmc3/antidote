...
`pin`::
  The pin annotation allows you to lock a bundle to a specific git ref such as a tag, branch, or commit SHA (eg: `pin:v1.0.0`). A pinned bundle will be cloned at the specified ref and will be skipped during `antidote update`. This is useful for preventing unwanted upstream changes from affecting your environment.
...
----
# pin a plugin to a specific tag
antidote bundle zsh-users/zsh-autosuggestions pin:v0.7.0
----
