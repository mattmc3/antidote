...
The default is `no`. Pinned bundles are always left shallow, since a pin already fixes the commit. Loading, updating, pinning, and restoring a snapshot all work on a shallow bundle. The one thing it gives up is the list of individual commits in the `antidote update` report: the old commit's parent is not present locally, so the report shows the before and after SHAs but not what came between. `antidote update` deepens any bundle it finds shallow unless that bundle is held shallow here, since the background fetch is best effort.
